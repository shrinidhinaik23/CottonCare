...
fun WelcomeScreen(navController: NavController) {
    Column(
        ...
        Column(horizontalAlignment = Alignment.CenterHorizontally) {
            Image(
                painter = painterResource(R.drawable.ic_cotton_logo),
                contentDescription = null,
                modifier = Modifier.size(96.dp)
            )
            Spacer(Modifier.height(16.dp))
            Text("Welcome to Cotton Care", fontWeight = FontWeight.Bold)
            Spacer(Modifier.height(8.dp))
            Text(
                "Your AI-powered companion for cotton disease detection and crop management",
                color = TextSecondary
            )
        }
        Row(
            ...
            PrimaryButton(text = "Next >", onClick = {
                navController.navigate(NavRoutes.Detect)
            })
        }
    }
}
