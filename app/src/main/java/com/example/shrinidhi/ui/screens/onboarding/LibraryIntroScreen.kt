...
@Composable
fun LibraryIntroScreen(navController: NavController) {
    Column(
        modifier = Modifier
            .fillMaxSize()
            .padding(24.dp),
        ...
        Column(horizontalAlignment = Alignment.CenterHorizontally) {
            ...
            Spacer(Modifier.height(16.dp))
            Text("Explore Disease Library", fontWeight = FontWeight.Bold)
            Spacer(Modifier.height(8.dp))
            Text(
                "Browse symptoms, causes, treatments, and prevention tips for each disease.",
                color = TextSecondary
            )
        }
        Row(
            ...
            PrimaryButton(text = "Next >", onClick = {
                navController.navigate(NavRoutes.RemediesIntro)
            })
        }
    }
}
